...
Through the MCP server you can also paste a **YouTube, X, LinkedIn** (and many other popular sites) link (public content): it's downloaded locally — the downloader is auto-fetched on first use — then transcribed. See [the MCP server README](packages/mcp-server#transcribe-from-a-link).
